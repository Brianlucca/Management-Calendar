...
import { LayoutDashboard, User, LogOut, Menu, Tags } from "lucide-react";
    ...
    <main className={`z-50 flex fixed top-0 left-0 h-full ${isOpen ? "w-64" : "w-20"} bg-gray-900 text-white p-4 transition-all duration-300`}>
      <nav className="w-full h-full flex flex-col">
        ...
        <ul className="space-y-4 flex-1">
          <li>
            <Link to="/" className="text-white flex items-center space-x-2 p-3 hover:bg-gray-800 rounded">
              <LayoutDashboard /> <span className={isOpen ? "block" : "hidden"}>Dashboard</span>
            </Link>
          </li>
          <li>
            <Link to="/profile" className="text-white flex items-center space-x-2 p-3 hover:bg-gray-800 rounded">
              <User /> <span className={isOpen ? "block" : "hidden"}>Perfil</span>
            </Link>
          </li>
          <li>
            <Link to="/tags" className="text-white flex items-center space-x-2 p-3 hover:bg-gray-800 rounded">
              <Tags /> <span className={isOpen ? "block" : "hidden"}>Gerenciar Tags</span>
            </Link>
          </li>
          <li>
            <button onClick={handleLogout} className="text-white flex items-center space-x-2 p-3 hover:bg-gray-800 rounded">
              <LogOut /> <span className={isOpen ? "block" : "hidden"}>Logout</span>
            </button>
          </li>
        </ul>
        <hr className="my-4 border-gray-700" />
        <div className={`text-center text-gray-500 text-sm ${isOpen ? "" : "whitespace-normal break-words"}`}>
